...
// mutex for output logs
static DEFINE_MUTEX(producer_log_mutex);
...
SYSCALL_DEFINE0(array_init)
{
    ...
    // open input file
    shared_file = filp_open(INPUT_FILE_NAME, O_RDONLY, 0);
    if (IS_ERR(shared_file)) {
        printk(KERN_ERR "Failed to open %s in array_init\n", INPUT_FILE_NAME);
        kfree(inv_arrays);
        kfree(circ_buffer);
        return PTR_ERR(shared_file);
    }

    // Initialize input file semaphore
...
SYSCALL_DEFINE0(producer_function)
{
    ...
    log_file = filp_open(PRODUCER_FILE_NAME, O_WRONLY | O_CREAT | O_APPEND, 0644);
    if (IS_ERR(log_file)) {
    ...
    while (1) { //continue reading from input file line by line untill end of the input file
        ...
        if (bytes_read <= 0) {
            printk(KERN_INFO "Producer: End of %s or error\n", INPUT_FILE_NAME);
            break;
        }
        ...
        mutex_unlock(&producer_log_mutex);
...
// to print all content in inventory_arrays to output log
